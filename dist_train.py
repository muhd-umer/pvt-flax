...
def train_and_evaluate(
    ...
    p_train_epoch = jax.pmap(
        partial(
            train_epoch, train_ds=train_ds, num_classes=num_classes, total=total_train
        ),
        axis_name="batch",
        donate_argnums=(0,),
    )

    p_test_epoch = jax.pmap(
        partial(test_epoch, test_ds=test_ds, num_classes=num_classes, total=total_test),
        axis_name="batch",
        donate_argnums=(0,),
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        state, train_loss, train_accuracy = p_train_epoch(state, dropout_rngs)

        test_loss, test_accuracy = p_test_epoch(state)
